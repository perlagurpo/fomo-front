    ...
    <nav className="sticky top-0 bg-fomo-sec-white border-gray-200 dark:fomo-pri-one" style={{ zIndex: '999' }}>
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <button onClick={() => setMenuOpened(!menuOpened)}
                type="button" 
                className="inline-flex items-center bg-transparent py-2 pr-2 rounded-lg md:hidden" aria-controls="navbar-default" aria-expanded="false">
        ...
        <div className={`${ menuOpened ? "block" : "hidden"} w-full md:hidden`} onClick={() => setMenuOpened(false)}>
          <ul className="flex flex-col text-center p-4 text-lg font-medium text-fomo-pri-one dark:bg-fomo-sec-white dark:text-fomo-pri-two">
            ...
            <li>
              <Link href="/nosotros" className="block py-2 pl-3 pr-4 rounded md:border-0 md:hover:text-orange-700 md:p-0  md:dark:hover:text-fomo-pri-one dark:hover:text-fomo-pri-one">
                Nosotros
              </Link>
            </li>
          </ul>
